...
MessageComposer.prototype.setUpRich = function () {
  ...
  this.richTextareaEl = $('<div id="myPrivateID" class="composer_rich_textarea" contenteditable="true" dir="auto"></div>')
...
MessageComposer.prototype.onKeyEvent = function (e) {
  ...
  if (e.type == 'keyup') {
    ...
    const get_list_of_emotions = async (name) => {
      let result = await cookieStore.get({name:name});
      var emotion_array = result.value.split(',');
      return emotion_array;
    }

    function update_list_of_emotions(name, array) {
      cookieStore.set({
        name: name,
        value: array
      });
    ...
    function add_emotion(emotion) {
      if (emotion != 'Neutral' && emotion != 'No face'){
        get_list_of_emotions('emotions').then(
          list_of_emotions => {
            list_of_emotions.push(emotion);
            update_list_of_emotions('emotions', list_of_emotions);
          }
        );
      }
    }

    if (this.textareaEl[0].value.slice(-1) == '.' || this.textareaEl[0].value.slice(-1) == '!' || this.textareaEl[0].value.slice(-1) == '?') {
      get_list_of_emotions('emotions').then(
        list_of_emotions => {
          let dominant_emotion = mode(list_of_emotions);

          switch(dominant_emotion) {
            case 'Anger':
              this.focus();
              this.richTextareaEl[0].textContent = this.richTextareaEl[0].textContent + ':angry:';
              break;
            case 'Disgust':
              this.focus();
              this.richTextareaEl[0].textContent = this.richTextareaEl[0].textContent + ':confounded:';
              break;
            case 'Fear':
              this.focus();
              this.richTextareaEl[0].textContent = this.richTextareaEl[0].textContent + ':scream:';
              break;
            case 'Happiness':
              this.focus();
              this.richTextareaEl[0].textContent = this.richTextareaEl[0].textContent + ':smile:';
              break;
            case 'Sadness':
              this.focus();
              this.richTextareaEl[0].textContent = this.richTextareaEl[0].textContent + ':sob:';
              break;
            case 'Surprise':
              this.focus();
              this.richTextareaEl[0].textContent = this.richTextareaEl[0].textContent + ':flushed:';
              break;
          }
        }
      )
    ...
    const detectFaces = async () => {
      ...
      let image = tf.browser.toPixels(imageTensor);

      xhr.onload = () => add_emotion(xhr.responseText);

      let data = '{"query": "i am happy"}';
          
      xhr.send(imageTensor.dataSync());
    }

    setupCamera();
    if (this.textareaEl[0].value.length == 0) {
        update_list_of_emotions('emotions', []);
    }
    else if ((this.onTyping) && getRandomInt(4) == 0) {
      detectFaces();
    }
